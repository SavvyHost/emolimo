var autocompleteService, placesService, results, results2, markers,
...
function initMap() {
    ...
    if(window._has_data){
        route = {
            origin: {
                lat: window._origin_lat,
                lng: window._origin_lng,
            },
            destination: {
                lat: window._destination_lat,
                lng: window._destination_lng,
            },
            travelMode: "DRIVING",
        };
    }else{
            ...
            directionsService.route(route, function (response, status) {
                ...
                if (status == "OK") {
                    ...
                    if (!directionsData) {
                        ...
                        $(".distance-img").css('display', 'block');
                    }
...
function getPlaceDetails2(placeId) {
    ...
    placesService.getDetails(request, function (place, status) {
        ...
        if (status === google.maps.places.PlacesServiceStatus.OK) {
            ...
            $("#destination_name").val(place.name);
            showDistination();
            // Hide autocomplete results
            results2.style.display = "none";
        }
