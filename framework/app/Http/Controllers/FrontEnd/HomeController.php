...
class HomeController extends Controller
{
    ...
    public function customer_register(Request $request)
    {
        ...
        if($request->has('flight_num'))
           $user->flight_num = $request->flight_num;

        if($request->has('baggage_details'))
           $user->baggage_details = $request->baggage_details;
...
//       dd($request->all());
       ...
       'return_journey' => 'nullable',
       'return_date' => 'required_if:return_journey,==,1',
       'return_time' => 'required_if:return_journey,==,1',
       ]);
       ...
       if ($validation->fails()) {
          ...
          $data = $request->all();
          $suitableVehicles = VehicleTypeModel::where('seats', '>=', $request->no_of_person)
          ->where('isenable', 1)
          ->get();
          foreach ($suitableVehicles as $vehicle){
             $base_fare = FareSettings::select('key_value as fare')
             ->where('key_name', 'like', "%_base_fare%")
             ->where('type_id', $vehicle->id)
             ->first();
             $base_km = FareSettings::select('key_value as km')
             ->where('key_name', 'like', "%_base_km%")
             ->where('type_id', $vehicle->id)
             ->first();
             $vehicle->km_price = $base_fare->fare / $base_km->km;
          }
          return view("frontend.journeyDetails", compact('suitableVehicles', 'data'));
       }
    }

    public function getNewJourney(){
       return view('frontend.journeyDetails');
    }

    public function getBookingSummary(Request $request){
       $vehicle = VehicleTypeModel::select('displayname', 'seats')
       ->where('isenable', 1)
       ->where('id', $request->vehicle_id)
       ->first();
       $data = $request->all();
       return view('frontend.summary', compact('data', 'vehicle'));
    }
    ...
    public function book(Request $request)
    {
        ...
        if (Auth::user() && Auth::user()->user_type == 'C') {
           ...
           'no_of_person' => 'required|integer',
           'return_journey' => 'nullable',
           'return_date' => 'required_if:return_journey',
           'return_time' => 'required_if:return_journey',
           ]);
   ...
   public function StoreBookingAndCustomer(Request $request)
   {
      $validator = Validator::make($request->all(), [
      'first_name' => 'required',
      'last_name' => 'required',
      ...
      if ($validator->fails()) {
         return back()->withErrors($validator, 'customer')->withInput();
      }
      ...
      if ($validation->fails()) {
         ...
         $booking->vehicle_typeid = $request->vehicle_type;
         $booking->save();
         Address::updateOrCreate(['customer_id' => $id, 'address' => $request->pickup_address]);
         Address::updateOrCreate(['customer_id' => $id, 'address' => $request->dropoff_address]);
         $this->book_later_notification($booking->id, $booking->vehicle_typeid);
         if (Hyvikk::email_msg('email') == 1) {
            Mail::to($booking->customer->email)->send(new VehicleBooked($booking));
         }
